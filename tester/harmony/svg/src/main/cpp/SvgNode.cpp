...
void SvgNode::InitStyle(const SvgBaseAttribute &attr) {
    ...
    if (hrefFill_) {
        ...
        if (!href.empty()) {
            ...
            auto pattern = GetPattern(href);
            if (pattern) {
                LOG(INFO) << "[UpdateCommonProps] fill state set pattern";
                attributes_.fillState.SetPattern(pattern.value(), true);
            } else {
...
std::optional<Pattern> SvgNode::GetPattern(const std::string &href) {
    if (!context_) {
        LOG(INFO) << "NO CONTEXT";
        return std::nullopt;
    }
    ...
    if (svgPattern) {
        return std::make_optional(svgPattern->GetPattern());
    }
    return std::nullopt;
}
